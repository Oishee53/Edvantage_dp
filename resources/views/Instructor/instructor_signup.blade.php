...
<body class="bg-teal-100 flex items-center justify-center min-h-screen">
  ...
  <div class="bg-white shadow-lg rounded-2xl w-full max-w-xl p-8">
    <h2 class="text-2xl font-bold text-center text-teal-800 mb-6">Instructor Sign Up</h2>
    ...
    <form action="{{ route('instructor.register') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
      ...
      <!-- Area of Expertise -->
        <div>
          <label class="block text-teal-700 font-medium mb-1">Area of Expertise</label>
          <input type="text" name="area_of_expertise" placeholder="e.g., Web Development, Data Science" value="{{ old('area_of_expertise') }}" required
                class="w-full border border-teal-300 rounded-lg p-3 focus:ring-2 focus:ring-blue-500 focus:outline-none">
        ...
        <!-- Qualification -->
        <div>
          <label class="block text-teal-700 font-medium mb-1">Qualification</label>
        ...
        <!-- Short Bio -->
        <div>
          <label class="block text-teal-700 font-medium mb-1">Short Bio</label>
          <textarea name="short_bio" rows="3" placeholder="Write a short introduction about yourself" required
                    class="w-full border border-teal-300 rounded-lg p-3 focus:ring-2 focus:ring-blue-500 focus:outline-none">{{ old('short_bio') }}</textarea>
          @error('short_bio')
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
          @enderror
        </div>
